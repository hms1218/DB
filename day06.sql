...
SELECT * FROM JOBS;
CALL MY_NEW_JOB_PRC('IT','DEV',1500,2000);
...
-- 결과가 없는 경우 : NO_DATA_FOUND 예외(오류)가 발생
-- 결과가 여러행인 경우 : TOO_MANY_ROWS 오류 발생
...
BEGIN
	...
	ELSE
		DBMS_OUTPUT.PUT_LINE('DONT HAVE JOBS'||' '||P_JOB_ID);
	END IF;
END;
...
-- 트리거(trigger)
-- 데이터베이스에서 특정 이벤트(INSERT,UPDATE,DELETE 같은 DML 또는 DDL작업)가 발생할 때
-- 자동으로 실행하는 PL/SQL문입니다.
 ...
 * (FOR EACH ROW) -> 행 단위 트리거가 되어 각 행마다 개별적으로 실행 (생략가능)
 * DECLARE
 * 	변수, 상수
 * BEGIN
...
-- 감사 및 로깅
-- 테이블의 변경 내역을 별도의 감사 테이블에 기록하여, 
-- 누가 언제 어떤 데이터를 변경했는지 추적할 수 있다.
-- 비즈니스로직 구현
-- ex)주문 테이블에 주문이 들어오면 상품테이블에서 자동을 재고를 수정한다.
-- 복잡한 데이터 처리
-- 한 번의 DML 작업으로 여러 관련 테이블의 동시 변경이 필요한 경우,
-- 트리거를 이러한 복잡한 로직을 중앙집중적으로 관리할 수 있다.

CREATE OR REPLACE TRIGGER TRG_CHECK_SALARY
BEFORE INSERT OR UPDATE ON EMPLOYEES -- EMPLOYEES 테이블에 추가나 수정이 발생하기 전에
FOR EACH ROW -- 영향을 받는 각 행마다 트리거가 실행
BEGIN
	IF :NEW.SALARY < 0 THEN -- :NEW -> 새로 삽입되거나 수정된 행의 값을 참조
...
VALUES(98, 'GILDONG', 'HONG', 'HONG1@GMAIL.COM', '111.1111', '2025.03.19', 'IT_PROG', 20000, 0, 100, 10);
...
UPDATE DEPT SET DNAME = '영업', LOC = 'INCHEON'
WHERE DEPTNO = 50;
