...
-- 단, 각 행의 모든 가능한 조합을 만들기 때문에 결과가 매우 큰 테이블이 될 수 있으므로 주의가 필요하다.
...
SELECT * FROM STADIUM S LEFT OUTER JOIN TEAM ON HOMETEAM_ID = TEAM_ID;
...
SELECT * FROM DEPARTMENTS; -- 27개 부서
SELECT FIRST_NAME, DEPARTMENT_ID FROM EMPLOYEES; -- 모든 직원이 11개의 부서에만 할당

SELECT FIRST_NAME, DEPARTMENT_NAME
FROM EMPLOYEES E RIGHT OUTER JOIN DEPARTMENTS D
ON E.DEPARTMENT_ID = D.DEPARTMENT_ID;

-- LEFT와 RIGHT 중에 뭘 많이 쓸까? -> 상황에 따라 다르지만 LEFT를 주로 쓴다.
...
SELECT * FROM PLAYER -- 팀아이디:TEAM_ID 선수이름:PLAYER_NAME
WHERE PLAYER_NAME = '송종국';

SELECT * FROM TEAM; -- 전화번호:TEL 팀아이디:TEAM_ID
...
SELECT * FROM JOBS; -- 직종번호:JOB_ID 직종이름:JOB_TITLE
SELECT * FROM EMPLOYEES; -- 이메일:EMAIL 이름:FIRST_NAME 성:LAST_NAME 직종번호:JOB_ID
...
-- 특정 값이 일정 범위 내에 속하거나, 두 값 사이의 관계를 비교하여 행을 결합할 때 유용하다.
...
SELECT * FROM DEPT; -- LOC , DEPTNO
SELECT * FROM EMP; -- SAL , DEPTNO

SELECT D.LOC, ROUND(AVG(SAL),0), SUM(SAL)
FROM DEPT D JOIN EMP E
ON D.DEPTNO = E.DEPTNO
GROUP BY D.LOC;

-- Natural Join
-- 두 테이블 간의 동일한 이름을 갖는 모든 컬럼들에 대해 등가조인을 수행한다.
...
-- VIEW
-- 하나 이상의 테이블이나 다른 뷰의 데이터를 볼 수 있게 해주는 데이터베이스의 객체이다.
-- 쿼리문을 작성하면 -> 1회성
...
-- 보안성
-- 숨기고 싶은 정보가 존재한다면, 뷰를 생성할 때 해당 컬럼은 빼고 생성함으로써 사용자에게 정보를 감출 수 있다.
...
CREATE OR REPLACE VIEW PLAYER_TEAM_NAME AS(
	SELECT T.TEAM_NAME, P.* FROM PLAYER P JOIN TEAM T ON P.TEAM_ID = T.TEAM_ID
);
...
CREATE OR REPLACE VIEW DATA_PLUS AS(
	SELECT FIRST_NAME||' '||LAST_NAME "NAME", SALARY, RANK() OVER (ORDER BY SALARY DESC) "RANK" FROM EMPLOYEES
);
...
-- PL(Procedural Language):절차적 언어/SQL문
...
BEGIN
	...
	ELSIF SCORE >= 80 THEN GRADE := 'C'; 
	ELSIF SCORE >= 80 THEN GRADE := 'D'; 
	ELSE GRADE := 'F';
	END IF;
...
BEGIN
	FOR REC IN EMP_CURSOR LOOP
		DBMS_OUTPUT.PUT_LINE('EMLPOYEE: '||REC.EMPLOYEE_ID||', NAME: '||REC.FIRST_NAME);
	END LOOP;
END;
...
-- 2. 프로시저(Procedures)
-- 데이터베이스에 저장되어 필요할 때마다 호출할 수 있는 이름이 있는 PL/SQL블록이다.
...
CREATE OR REPLACE PROCEDURE MY_NEW_JOB_PRC(
...
-- 프로시저 호출
-- 한 번 실행했을 때는 추가가 됨
-- 두 번 실행하면 PK 제약조건에 걸려서 실행이 안됨
CALL MY_NEW_JOB_PRC('IT','DEVELOPER',14000,20000);
